using System.IO;
using System.Linq;
using Cortside.AmqpTools.WebApi.IntegrationTests.Data.Ids;
using Newtonsoft.Json;
using WireMock.RequestBuilders;
using WireMock.ResponseBuilders;
using WireMock.Server;

namespace Cortside.AmqpTools.WebApi.IntegrationTests.Helpers.IDSMock {
    public class IdsMock {
        ...
        public IdsMock(WireMockServer server) {
            ...
            idsConfiguration.Authorization_endpoint = idsConfiguration.Authorization_endpoint.Replace("https://identityserver.k8s.acme.com", MockServer.Urls.First());
            idsConfiguration.Issuer = idsConfiguration.Issuer.Replace("https://identityserver.k8s.acme.com", MockServer.Urls.First());
            idsConfiguration.Jwks_uri = idsConfiguration.Jwks_uri.Replace("https://identityserver.k8s.acme.com", MockServer.Urls.First());
            idsConfiguration.Token_endpoint = idsConfiguration.Token_endpoint.Replace("https://identityserver.k8s.acme.com", MockServer.Urls.First());
            idsConfiguration.Userinfo_endpoint = idsConfiguration.Userinfo_endpoint.Replace("https://identityserver.k8s.acme.com", MockServer.Urls.First());
            idsConfiguration.End_session_endpoint = idsConfiguration.End_session_endpoint.Replace("https://identityserver.k8s.acme.com", MockServer.Urls.First());
            idsConfiguration.Check_session_iframe = idsConfiguration.Check_session_iframe.Replace("https://identityserver.k8s.acme.com", MockServer.Urls.First());
            idsConfiguration.Revocation_endpoint = idsConfiguration.Revocation_endpoint.Replace("https://identityserver.k8s.acme.com", MockServer.Urls.First());
            idsConfiguration.Introspection_endpoint = idsConfiguration.Introspection_endpoint.Replace("https://identityserver.k8s.acme.com", MockServer.Urls.First());
            idsConfiguration.Device_authorization_endpoint = idsConfiguration.Device_authorization_endpoint.Replace("https://identityserver.k8s.acme.com", MockServer.Urls.First());
        ...
        public void Configure() {
            ...
            MockServer
                .Given(
                Request.Create().WithPath($"/runtime/policy/Amqp*")
                    .UsingPost()
            )
            .RespondWith(
                Response.Create()
                    .WithStatusCode(200)
                    .WithBody(r => $"{{\"roles\":[\"Admin\"],\"permissions\":[\"GetMessagesByQueue\",\"ShovelQueues\",\"DeleteMessage\", \"GetQueueRuntimeInfo\"]}}")
            );
